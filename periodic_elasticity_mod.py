...
def build_deformation_meshes(u, v, scale=1.0):
    ...
    # undeformed mesh
    undeformed = pyvista.UnstructuredGrid(
        topology,
        cell_types,
        geometry,
    )
    ...
    # deformed mesh
    deformed = pyvista.UnstructuredGrid(
        topology,
        cell_types,
        geometry.copy(),
    )
    ...
    warped = deformed.warp_by_vector(
        "u",
        factor=scale,
    )
...
for istep, strain in enumerate(macro_strains):
    ...
    # u_macro = fem.Function(mpc.function_space)
    ...
    # u.x.array[:] = u_macro.x.array
    u.x.array[:] += v.x.array[:]
    
    undeformed, warped = build_deformation_meshes(
        u,
        v,
        scale=5.0,
    )
