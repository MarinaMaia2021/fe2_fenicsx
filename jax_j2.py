...
def constitutive_update_batch(eps_new_batch, hist_state, material):
    ...
    # CRITICAL FIX: Direct fallback if JAX has flattened it into a standard tuple
    if isinstance(hist_state, tuple) and not hasattr(hist_state, "eps_plastic"):
        hist_state = HistState(eps_plastic=hist_state[0], eps_p_eq=hist_state[1])
