...
STATIC_MAX_ITER_YIELD = 8  # Fixed number of iterations for the return mapping scheme. Significantly influences performance. For jit compilation, this should be a constant.
...
def newton_step(dgam, old_dgam, old_yield_val, A11_tr, A22_tr, A33_tr, eps_p_eq_0, material):
    ...
    safe_der = jnp.where(jnp.abs(yield_der) < 1e-12, jnp.sign(yield_der) * 1e-12, yield_der)
    ddgam = yield_val / safe_der
    ...
    new_dgam = dgam - ddgam
    new_dgam = jnp.clip(new_dgam, min = 0.0)
...
def find_root(A11_tr, A22_tr, A33_tr, eps_p_eq_0, material, is_plastic_mask):
    """Find root of yield function using Newton-Raphson method."""
    dgam_init = jnp.array(1e-10)
    old_ddgam_init = jnp.array(-1.0)

    # CRITICAL JAX FIX: If this is an elastic perturbation, feed the solver safe, fake invariants
    # so it does not divide by zero or look for roots of negative yields.
    safe_A11 = jnp.where(is_plastic_mask, A11_tr, 100.0)
    safe_A22 = jnp.where(is_plastic_mask, A22_tr, 100.0)
    safe_A33 = jnp.where(is_plastic_mask, A33_tr, 100.0)

    xi_tr = safe_A11 / 6.0 + 0.5 * safe_A22 + 2.0 * safe_A33
    old_yield_val_init = 0.5 * xi_tr - sigma_C(eps_p_eq_0, material) ** 2 / 3.0
    
    # If it's a dummy pass, force the yield value to appear perfectly solved (0.0)
    old_yield_val_init = jnp.where(is_plastic_mask, old_yield_val_init, 0.0)

    # Initial state
    init_val = (dgam_init, old_ddgam_init, old_yield_val_init, 0, False, 
                safe_A11, safe_A22, safe_A33, eps_p_eq_0, material)

    # Run iterations safely
    ...
    # Overwrite outputs if this was a dummy evaluation
    dgam = jnp.where(is_plastic_mask, dgam, 0.0)
    converged = jnp.where(is_plastic_mask, converged, True)

    # Return dgam, eps_p_eq and convergence flag
    _, eps_p_eq = eval_yield(dgam, safe_A11, safe_A22, safe_A33, eps_p_eq_0, material)
    eps_p_eq = jnp.where(is_plastic_mask, eps_p_eq, eps_p_eq_0)
...
#     is_tr_plastic = old_yield_val_init >= material.return_map_tol
#     old_yield_val_init = jnp.where(is_tr_plastic, old_yield_val_init, 0.0)
...
#     # Force dgam to be absolutely 0 if it was originally an elastic state
#     dgam = jnp.where(is_tr_plastic, dgam, 0.0)
...
#    jax_print("eps_new {} eps_p_hist {}", eps_new, eps_p_hist )
    ...
    def plastic_update():
        # CRITICAL JAX FIX: If this branch is evaluated as a dummy track during 
        # JAX's automatic differentiation perturbation pass, swap the raw 
        # invariants with safe, strictly positive nominal numbers (e.g., 100.0).
        # This keeps jnp.sqrt(xi) and its derivative clean and prevents NaN.
        safe_A11 = jnp.where(is_plastic, A11_tr, 100.0)
        safe_A22 = jnp.where(is_plastic, A22_tr, 100.0)
        safe_A33 = jnp.where(is_plastic, A33_tr, 100.0)

        # Find root using Newton-Raphson with the sanitized invariants
        dgam, new_eps_p_eq, _ = find_root(safe_A11, safe_A22, safe_A33, eps_p_eq_hist, material, is_plastic)
                
        # Convert to 3D representation for stress calculations
        sig_tr_3 = t6_to_t3(sig_tr)

        # Force dgam to 0.0 if this is a dummy tracking pass
        safe_dgam = jnp.where(is_plastic, dgam, 0.0)
        A_mat = get_A_matrix(safe_dgam, material)
        stress_3 = jnp.matmul(A_mat, sig_tr_3)
        sig = t3_to_t6(stress_3)

        # Compute plastic strain increment
        depsp3 = safe_dgam * jnp.matmul(material.P, stress_3)
        depsp = ep3_to_ep6(depsp3)

        # Mask the history updates if it isn't truly plastic
        new_eps_p = jnp.where(is_plastic, eps_p_hist + depsp, eps_p_hist)
        final_eps_p_eq = jnp.where(is_plastic, new_eps_p_eq, eps_p_eq_hist)

        # Return stress, new plastic strain and equivalent plastic strain
        stress = jnp.zeros_like(eps_new)
        stress = stress.at[0].set(jnp.where(is_plastic, sig[0], sig_tr[0]))
        stress = stress.at[1].set(jnp.where(is_plastic, sig[1], sig_tr[1]))
        stress = stress.at[2].set(jnp.where(is_plastic, sig[3], sig_tr[3]))
        
        # Ensure your custom tracking prints output the real or masked status cleanly
       # jax_print("prev pl. {}, pl. {}, prev eq. pl. {} current eq. pl.strain {}", eps_p_hist, new_eps_p, eps_p_eq_hist, new_eps_p_eq)
        
        return stress, new_eps_p, final_eps_p_eq
...
#     jax_print("eps p hist {}", eps_p_hist)
...
#     # jax.lax.cond(
#     #     is_plastic,
#     #     lambda: jax_print("Point is plastic: {}", f_tr),
#     #     lambda: jax_print("Point is elastic: {}", f_tr),
#     # )

#     # Function for plastic update
#     def plastic_update():
#         # Find root using Newton-Raphson
#         dgam, new_eps_p_eq, _ = find_root(A11_tr, A22_tr, A33_tr, eps_p_eq_hist, material, is_plastic)
     
#         # jax.lax.cond(
#         #     jnp.isnan(dgam),
#         #     lambda: jax_print("NAN -> dgam: {}, prev. {} eq. pl.strain: {}", dgam, eps_p_eq_hist, new_eps_p_eq),
#         #     lambda: None
#         # )
...
#         jax_print("{}, plastic {}, prev. {} eq. pl.strain {}", is_plastic, new_eps_p, eps_p_eq_hist, new_eps_p_eq)
